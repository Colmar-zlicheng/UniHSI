...
def VaryPoint(data, axis, degree):
    xyzArray = {
        'X':
            np.array([[1, 0, 0], [0, np.cos(np.radians(degree)), -np.sin(np.radians(degree))],
                      [0, np.sin(np.radians(degree)), np.cos(np.radians(degree))]]),
        'Y':
            np.array([[np.cos(np.radians(degree)), 0, np.sin(np.radians(degree))], [0, 1, 0],
                      [-np.sin(np.radians(degree)), 0, np.cos(np.radians(degree))]]),
        'Z':
            np.array([[np.cos(np.radians(degree)), -np.sin(np.radians(degree)), 0],
                      [np.sin(np.radians(degree)), np.cos(np.radians(degree)), 0], [0, 0, 1]])
    }
...
class UniHSI_PartNet_BKP(humanoid_amp_task.HumanoidAMPTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.joint_name = [
            "pelvis", "left_hip", "left_knee", "left_foot", "right_hip", "right_knee", "right_foot", "torso", "head",
    ...
    def _load_mesh(self):
        ...
        for plans in self.plan_items:
            ...
            for obj_id in objs:
                ...
                min_x_mesh, min_y_mesh, min_z_mesh = mesh_vertices_single[:, 0].min(), mesh_vertices_single[:, 1].min(
                ), mesh_vertices_single[:, 2].min()
        ...
        obj_rotate_matrix = np.array([[np.cos(np.radians(obj_rotate)), -np.sin(np.radians(obj_rotate)), obj_rotate * 0],
                                      [np.sin(np.radians(obj_rotate)),
                                       np.cos(np.radians(obj_rotate)), obj_rotate * 0],
        ...
        for i in range(self.num_scenes_row):
            for j in range(self.num_scenes_col):
                ...
                self.gym.add_triangle_mesh(self.sim, mesh_vertices_offset.flatten(order='C'),
                                           mesh_triangles.flatten(order='C'), tm_params)
        ...
        for p in contact_parts:
            if 'floor' in p:
                ...
                if object[:3] == "bed":
                    ...
                    mask = ((out_part_pcd[:, 0] > max_x) |
                            (out_part_pcd[:, 0] < min_x)) | ((out_part_pcd[:, 1] > max_y) |
                                                             (out_part_pcd[:, 1] < min_y))
                out_part_pcd = out_part_pcd[mask]
            else:
                idx = label_mapping[p]
                part_pcd = pcd[label == idx]
                max_x, min_x, max_y, min_y, max_z, min_z = part_pcd[:, 0].max(), part_pcd[:, 0].min(
                ), part_pcd[:, 1].max(), part_pcd[:, 1].min(), part_pcd[:, 2].max(), part_pcd[:, 2].min()

                out_part_pcd = part_pcd[(part_pcd[:, 0] <= max(
                    (max_x - 0.2),
                    (max_x - min_x) / 5 * 4 + min_x)) & (part_pcd[:, 0] >= min(min_x + 0.2,
                                                                               (max_x - min_x) / 5 * 1 + min_x)) &
                                        (part_pcd[:, 1] <= max((max_y - 0.2), (max_y - min_y) / 5 * 4 + min_y)) &
                                        (part_pcd[:, 1] >= min(min_y + 0.2,
                                                               (max_y - min_y) / 5 * 1 + min_y))]  # filter edge
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        self.x_offset = (self.env_array % num_per_row % self.num_scenes_row) * spacing * 2 - (self.env_array %
                                                                                              num_per_row) * spacing * 2
        self.y_offset = (self.env_array // num_per_row %
                         self.num_scenes_col) * spacing * 2 - (self.env_array // num_per_row) * spacing * 2

        # [num_envs, num_obj, num_part_sequence, num_pts. 3]
        self.envs_obj_pcd_buffer = self.obj_pcd_buffer.new_zeros(
            [self.num_envs, self.obj_pcd_buffer.shape[2], self.obj_pcd_buffer.shape[3], self.obj_pcd_buffer.shape[4]])
        ...
        self.scene_stand_point = torch.einsum("nmoe,neg->nmog", self.scene_stand_point[self.scene_for_env],
                                              self.obj_rotate_matrix[self.env_scene_idx_row, self.env_scene_idx_col])
        self.scene_stand_point[..., 0] += self.x_offset[:, None,
                                                        None] + self.rand_dist_x[self.env_scene_idx_row,
                                                                                 self.env_scene_idx_col][:, None, None]
        self.scene_stand_point[..., 1] += self.y_offset[:, None,
                                                        None] + self.rand_dist_y[self.env_scene_idx_row,
                                                                                 self.env_scene_idx_col][:, None, None]
        ...
        self.envs_heightmap = torch.einsum("nae,neg->nag", self.envs_heightmap,
                                           self.obj_rotate_matrix[self.env_scene_idx_row, self.env_scene_idx_col])
    ...
    def _reset_target(self, env_ids, success):
        ...
        self._humanoid_root_states[
            env_ids[reset], 0] += self.x_offset[env_ids[reset]] + x_sign[env_ids[reset]] * rand_dist_x[env_ids[reset]]
        self._humanoid_root_states[
            env_ids[reset],
            1] += self.y_offset[env_ids[reset]] + y_sign[env_ids[reset]] * rand_dist_y[env_ids[reset]] - 2
    ...
    def _compute_task_obs(self, env_ids=None):
        ...
        obs, self.local_height_map, self.rotated_mesh_pos = compute_strike_observations(
            root_states, tar_pos, joint_pos_buffer, pcd_buffer, tar_rot, tar_vel, tar_ang_vel, contact_type,
            contact_valid, contact_direction, self.humanoid_in_mesh, origin_root_pos, self.local_scale, height_map,
            self.mesh_pos, tar_dir)
        ...
        self.rew_buf[:], self.location_diff_buf[:], self.joint_diff_buff[:], self.joint_idx_buff[:], self.tar_dir[:, :2] = compute_contact_reward(
            target, char_root_state, pcd_buffer, joint_pos_buffer, self._prev_root_pos, self.dt, self.contact_type,
            self.contact_valid, self.contact_direction)
    ...
    def _compute_reset(self):
        ...
        self.reset_buf[:], self._terminate_buf[:], self.still_buf[:] = compute_humanoid_reset(
            self.reset_buf, self.progress_buf, self._contact_forces, self._contact_body_ids, self._rigid_body_pos,
            self._strike_body_ids, self.max_episode_length, self._enable_early_termination, self._termination_heights,
            self._rigid_body_vel, self.still_buf, fulfill, self.big_force)
        return
    ...
    def _draw_task(self):
        ...
        joints_contact = joint_pos_buffer.view(-1, 3)[joint_contact_choice.view(-1)].clone()
        ends[valid_joint_contact_choice.view(-1)[self.contact_valid[0]]] = joints_contact[
            valid_joint_contact_choice.view(-1)]
...
# @torch.jit.script
def compute_strike_observations(root_states, tar_pos, joint_pos_buffer, pcd_buffer, tar_rot, tar_vel, tar_ang_vel,
                                contact_type, contact_valid, contact_direction, human_in_mesh, origin_root_pos,
                                local_scale, height_map_pcd, mesh_pos, tar_dir):
    ...
    heading_rot_for_height[:, 2] = heading_rot_for_height[:, 2] * heading_rot_norm / torch.abs(
        heading_rot_for_height[:, 2])
    ...
    obs = torch.cat([local_tar_pos, local_tar_rot_obs, local_tar_vel, tar_dir, contact_type, contact_valid, height_map],
                    dim=-1)

    local_target_pos = pcd_buffer - joint_pos_buffer
    local_target_pos_r = quat_rotate(heading_rot[:, None].repeat(1, local_target_pos.shape[1], 1).view(-1, 4),
                                     local_target_pos.view(-1, 3))
...
def compute_contact_reward(target, root_state, pcd_buffer, joint_pos_buffer, prev_root_pos, dt, contact_type,
                           contact_valid, contact_direction):
    ...
    for i in range(pcd_buffer.shape[1]):
        ...
        near_pos_reward_contact = near_pos_reward * contact_type[:,
                                                                 i] + (1 - near_pos_reward) * (1 - contact_type[:, i])

        reward_dir = torch.nn.functional.normalize(-near_pos_diff[range(near_pos_diff.shape[0]), min_pos_idx],
                                                   dim=-1) * contact_direction[:, i]
...
# @torch.jit.script
def compute_humanoid_reset(reset_buf, progress_buf, contact_buf, contact_body_ids, rigid_body_pos, strike_body_ids,
                           max_episode_length, enable_early_termination, termination_heights, _rigid_body_vel,
                           still_buf, fulfill, big_force):
    ...
    terminated = torch.where(big_force, torch.ones_like(reset_buf),
                             terminated)  # terminate when force is too big (could cause peneration)
